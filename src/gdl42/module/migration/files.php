...
if (file_exists("./files/misc/files.lck")){
	...
	$folder1=isset($_GET["folder1"]) ? $_GET["folder1"] : null;
	$folder2=isset($_GET["folder2"]) ? $_GET["folder2"] : null;
	$submit=isset($_POST["submit"]) ? $_POST["submit"] : null;
	$lock=isset($_GET["lock"]) ? $_GET["lock"] : null;
	$temp=getcwd();
	if (!empty($folder1)) {
	...
	if (!empty($submit)) {
		$window=isset($_POST["window"]) ? $_POST["window"] : null;
