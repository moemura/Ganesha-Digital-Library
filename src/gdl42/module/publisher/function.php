...
function add_publisher_form($action="") {
	...
	$key=array_keys(array("INSTITUTION","PERSONAL","WARNET"),strtoupper($frm["type"]));		
	$gdl_form->add_field(array(
				"type"=>"radio",
				"name"=>"frm[type]",
	...
	$key=array_keys(array("dedicated","temporary"),strtolower($frm["contype"]));	
	$gdl_form->add_field(array(
				"type"=>"radio",
				"name"=>"frm[contype]",
